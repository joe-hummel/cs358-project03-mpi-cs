...
uchar **ReadBitmapFile(char *filename, BITMAPFILEHEADER &bitmapFileHeader, BITMAPINFOHEADER &bitmapInfoHeader, int &rows, int &cols)
{
	...
	//
	// NOTE: sometimes the image size is 0, leave it alone.
	//
	...
	//
	// sometimes image_size is 0, so we have to compute ourselves:
	//
	uint32_t imageSize = bitmapInfoHeader.image_size;

	if (imageSize == 0) {
		imageSize = bitmapInfoHeader.height * bitmapInfoHeader.width * 3 /*RGB*/;
	}
	...
	fread(image[0], imageSize /*bitmapInfoHeader.image_size*/, 1, filePtr);
...
void WriteBitmapFile(char *filename, BITMAPFILEHEADER bitmapFileHeader, BITMAPINFOHEADER bitmapInfoHeader, uchar **image)
{
	...
	fwrite(bitmapImage, imageSize /*bitmapInfoHeader.image_size*/, 1, filePtr);
